...
| 스킬 | 언제 발동 | 하는 일 |
|---|---|---|
| `planning-interview` | "새 앱 기획 시작", 아이디어 제시 | 질문 하나씩 함께 기획 → PROJECT.md 채움 → 모듈 매핑 (진실원천: `playbook/01·02`) |
| `design-polish` | UI를 만들거나 다듬을 때("디자인 다듬어줘", 데모 직전) | 테마·토큰·슬롭 회피·라이팅(토스 기준)·출력 계약을 한 번에 점검·적용 |
| `demo-prep` | "데모 준비", 제출·발표 직전 | 검증 시나리오 → 60~90초 피치 스크립트 + 사고 예방 체크 + 리허설 (진실원천: `playbook/03`) |
