...
## 5b. Happy Path (해피패스 유저저니 · 사람↔AI 공용 기준)
- 5칸 이내로(텍스트/ASCII): **도착 상태 → ① … → ② … → ③ … → 완주.** 각 칸 = "무엇이 뜨나 + 사용자가 무엇을 하나".
- 이 한 장이 *정렬(빌드 브리프) · 검증(완주=§10) · 발표(해결 후 모습) · 데모 대본*을 겸한다.
- ※ 와이어프레임/정보구조도(IA)/플로우차트는 **안 그린다** — 패턴(`patterns/`)+템플릿+`tokens.css`가 기본값, AI가 진짜 화면으로 대체.
> 
