...
## '원칙' 문서 3종 — 역할 구분 (헷갈리지 않게)
- **`CLAUDE.md` 5대 Rule** = *무엇을 만들까*(제품 행동 원칙: 행동>정보, 선택>입력 …)
- **`lessons/README` 체크리스트** = *UI를 만들 때* 항상 적용할 UX 기본값
- **`research/build-principles.md`** = *어떻게 빌드하나*(딥리서치 7원칙: 컨텍스트·검증 루프 …)
> 셋은 층위가 다르다(제품 / UI / 빌드방법). 충돌이 아니라 보완.

## 누가 무엇을 보나
| | 사람(기획자/리뷰어) | AI(Claude) |
|---|---|---|
| `01-planning-interview` | 질문에 답하며 생각 정리 | 질문을 던지고 답을 템플릿에 채움 |
| `02-project-template` | 기획서로 검토·승인 | 빌드의 ground truth로 사용 |
...
```
아이디어
 1) 함께 기획   → 01-planning-interview (AI가 질문 → 사람이 답 → 함께 좁힘)
 2) 기획 확정   → 02-project-template 채움 (1~10 = 기획, 11 = 개발 지시서)
