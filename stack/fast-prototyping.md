...
- **4유형(A·B·C·D) 전체 이식 검증됨** — 병렬 에이전트 3개가 llms-ctx 하네스+작동 예제 2개만으로 각 4분39초~5분45초에 완성(환각 0). 로직(patterns 메커니즘)은 4종 모두 **무수정 이식** — 상황트리 칩·벌크 서류(A), 트라이스테이트·모름 가산(B), D-day·HTMX 완료 토글(C), 라우팅·갭점검(D).
- **멀티페이지 위저드 전환도 4종 검증됨**(각 4~7분, 로직 무수정) — 단, **유형별 적정 단계 수가 다르다**: A=5단계(조건부 스킵 포함) / B·D=3단계 / **C(체크리스트)=2단계가 적정**(리스트 자체가 본체라 과한 위저드는 역효과). 페이지 안 상호작용(완료 토글 등)은 HTMX 부분갱신으로 페이지를 떠나지 않게.
...
- **Windows에서 한글 POST 스모크 테스트** — curl/PowerShell 콘솔(cp949)이 한글 바디를 깨뜨려 오판 유발. HTTP 검증은 httpx 스크립트나 브라우저(Playwright)로.
